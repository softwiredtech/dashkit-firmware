#!/usr/bin/env bash
# Run the Tesla crypto known-answer unit test (tools/test/test_tesla_crypto.c)
# against the vendored crypto/session/protobuf layers and a pinned host
# mbedTLS 3.6.2 (built by the shared tesla_host_env.sh bootstrap).
#
...
cc -std=c99 -Wall -Wextra \
   ...
   -o "${MBEDTLS_WORK}/test_tesla_crypto"

echo "==> Running test"
"${MBEDTLS_WORK}/test_tesla_crypto"
